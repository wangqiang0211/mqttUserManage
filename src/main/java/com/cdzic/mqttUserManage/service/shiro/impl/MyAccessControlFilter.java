...
public class MyAccessControlFilter extends AccessControlFilter {
    ...
    //设置Cache的key的前缀
    public void setCacheManager(CacheManager cacheManager,String projectName) {
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        ...
        if (null==deque){
            ...
            deque.push(sessionId.toString());
            cache.put(username,deque);
        }else if (!deque.contains(sessionId) && session.getAttribute("kickout") == null) {
            //将sessionId存入队列
            deque.push(sessionId.toString());
            //将用户的sessionId队列缓存
            cache.put(username, deque);
        ...
        while (deque.size() > maxSession) {
            ...
            } catch (Exception e) {//ignore myexception
